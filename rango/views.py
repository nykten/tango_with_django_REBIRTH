...
def show_category(request, category_name_slug):
    ...
    try:
        ...
        # The filter() will return a list of page objects or an emmpty list.
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        # Note that we make use both UserForm and UserProfileForm.
...
def user_login(request):
    # If teh request is a HTTP POST, try to pull out the relevant information.
    if request.method == 'POST':
        # Gather the username and passowrd provided by the user.
        # This information is obtained from the login form.
        # We use request.POST.get('<variable>') as opposed
        # to request.POST.get['<variable>'], because the
        # request.POST.get('<variable>') returns None if the
        # value does not exist, while request.POST.get['<variable>']
...
def visitor_cookie_handler(request,response):
    ...
    # If the cookie doesn't exit, then default value of 1 is used.
